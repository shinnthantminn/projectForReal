...
export default class VideoPlayer extends Component {
  ...
  hideControlAnimation() {
    Animated.parallel([
      ...
      Animated.timing(this.animations.customControl.opacity, {
        toValue: 0,
        duration: this.props.controlAnimationTiming,
        useNativeDriver: false,
      }),
      Animated.timing(this.animations.customControl.marginTop, {
        toValue: 400,
        duration: this.props.controlAnimationTiming,
        useNativeDriver: false,
      })
   ...
   */
  renderFullscreen() {
    let source = require('./assets/img/fullscreen.png');
  ...
  renderCustomControls() {
    const fastBackwardControl = this.props.disableFastBackward
      ? this.renderNullControl()
      : this.renderFastBackward();

    const fastForwardControl = this.props.disableFastForward
      ? this.renderNullControl()
    ...
    return (
      <Animated.View
        style={[
          styles.controls.custom,
          {
            opacity: this.animations.customControl.opacity,
            marginTop: this.animations.customControl.marginTop
          },
        ]}>
        <SafeAreaView
          style={[styles.controls.row, styles.controls.customControlGroup]}>
          {fastBackwardControl}
          {playPauseControl}
          {fastForwardControl}
        </SafeAreaView>
  ...
  renderPlayPause() {
    let source =
      this.state.paused === true
        ? require('./assets/img/play1.png')
        : require('./assets/img/pause1.png');
    return this.renderControl(
      <Image source={source} />,
      this.methods.togglePlayPause,
   ...
   */
  renderFastBackward() {
    let source = require('./assets/img/backward.png');
    return (
      <TouchableHighlight
        underlayColor="transparent"
        activeOpacity={0.3}
        onPressIn={() => {
          ...
          let currentTime = this.calculateTimeFromSeekerPosition();
          if(currentTime <= 10) {
            currentTime = 0;
          } else {
        ...
        style={[styles.controls.control, styles.controls.fastForwardBackwardControl]}>
        <Image source={source} />
      </TouchableHighlight>
    );
  }
  ...
  renderTimer() {
    return this.renderControl(
      <Text style={[styles.controls.timerText, this.props.timerText]}>{this.calculateTime()}</Text>,
